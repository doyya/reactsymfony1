...
const UserList = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        ...
        const userResponse = await fetch(`https://127.0.0.1:8000/api/users/${id}`);
        const userData = await userResponse.json();
        setUser(userData);

        // Récupérer les possessions de l'utilisateur
        const possessionsResponse = await fetch(`https://127.0.0.1:8000/api/users/${id}/possessions`);
  ...
  return (
    <div>
      <h2 className='userdetails'>Les détails utilisateur</h2>
